...
async function evaluateMessage(message) {
  ...
  for (const attribute in emojiMap) {
    if (scores[attribute]) {
      ...
      let modlog = client.channels.cache.get('841250681109348362');
      modlog.send(`${message.author} has been warned for toxicity/spam ${emojiMap[attribute]} at ${message.id}`);
